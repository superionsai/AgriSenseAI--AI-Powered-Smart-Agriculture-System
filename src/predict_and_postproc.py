...
def generate_llm_summary(result_json, model_name="llama:7b"):
    """
    Call Ollama (llama:7b) to generate a post-processed advisory.
    Ensures JSON output format by wrapping raw response.
    """
    schema = {
        "summary": "",
        "risks": [],
    ...
    prompt = (
        "You are an agricultural expert. Analyze the following forecast and advisory.\n"
        "Write a concise farmer-friendly analysis including:\n"
        "1. Summary of weather trends (rain, temperature, humidity, etc.)\n"
        "2. Possible risks (drought, fungal disease, heat stress, etc.)\n"
    ...
    try:
        ...
        raw_out, err = process.communicate(prompt, timeout=180)

        if err:
            return {**schema, "summary": f"Ollama error: {err.strip()}"}

        if not raw_out.strip():
            return {**schema, "summary": "⚠️ Ollama returned empty response."}

        # Always wrap raw text into JSON schema
        return {
            "summary": raw_out.strip(),
            "risks": [],
            "recommendations": [],
            "actions": [],
        }
        ...
        return {**schema, "summary": f"LLM generation failed: {e}"}


def evaluate_and_advisory(
    df_proc_path, scaler_path, model, in_len=30, out_len=15, device="cpu"
):
    ...
    # Add llama3/llama:7b analysis
    result["llm_analysis"] = generate_llm_summary(result, model_name="llama:7b")
    return result
...
if __name__ == "__main__":
    ...
    advisory = evaluate_and_advisory(
        proc_csv, scaler_file, model, in_len=30, out_len=OUT_LEN, device=device
    )
