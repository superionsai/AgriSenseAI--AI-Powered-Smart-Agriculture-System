...
def make_sequences(df, in_len=30, out_len=15):
    """Convert dataframe into overlapping input/output sequences."""
    arr = df.values
    n = len(arr)

    if n < in_len + out_len:
        raise ValueError(
            f"Not enough data: only {n} rows, need at least {in_len + out_len}"
        )
...
if __name__ == "__main__":
    ...
    # Load first processed dataset
    df = pd.read_csv(
        os.path.join(PROC_DIR, proc_files[0]), index_col=0, parse_dates=True
    )
    ...
    if len(df) < IN_LEN + OUT_LEN:
        print(
            f"⚠️ Dataset too short ({len(df)} rows). "
            f"Adjusting IN_LEN and OUT_LEN..."
        )
