...
def train_model(train_loader, val_loader, model, epochs=40, lr=1e-3, device="cpu"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            # Defensive casts and device move
            xb = xb.to(device=device, dtype=torch.float32)
            yb = yb.to(device=device, dtype=torch.float32)

            # Sanity check shapes
            if xb.dim() != 3 or yb.dim() != 3:
                raise RuntimeError(f"Unexpected batch shapes xb:{xb.shape} yb:{yb.shape}")
            ...
            try:
                ...
                # helpful debug info
                print("Model forward failed. xb.shape:", xb.shape, "yb.shape:", yb.shape)
                raise
...
if __name__ == "__main__":
    ...
    _ = train_model(train_loader, val_loader, model, epochs=25, lr=1e-3, device=device)
